---
name: trae-bridge
description: Trae技能桥接 — 自动发现和加载 Trae 的 62 个技能。当需要数据库操作、Docker管理、GitHub CLI、文档处理、量化交易、微信发布等能力时，自动从 Trae 技能库查找并应用。
---

# Trae 技能桥接

本技能作为 Codex 和 Trae 技能系统之间的桥梁。当需要某项能力时，先检查下方索引找到对应的 Trae 技能，然后读取其 SKILL.md 获取详细指令。
...
| ppt-master | 网页PPT/演示文稿生成 | 制作演示文稿 |
...
| xiaohongshu-creator | 小红书全能创作者（搜索/文案/生图/发布/多账号管理） | 小红书内容创作、发布、账号管理 |

### 元技能与工具
| 技能名 | 描述 | 触发场景 |
|--------|------|---------|
| `skill-creator` | 创建/更新Codex技能的指南 | 创建新技能 |
...
| `search-skill` | 从可信市场搜索和推荐Codex技能 | 按需求查找技能 |
...
- **优先使用 Codex 原生能力**：ECC 技能系统和官方插件已覆盖大量场景，Trae 技能作为补充
